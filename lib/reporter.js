...
JazzHandsReporter.prototype.output = function (str) {
  print(str);
};
...
JazzHandsReporter.prototype.reportRunnerResults = function (runner) {
  ...
  summaryStr = results.failedCount ? summaryStr + results.failedCount + ' failed':summaryStr;
  this.output(summaryStr);
  if (results.failedCount > 0) {
    var failMessages = [];
    var failOutput = '';
    for (key in this.specResults) {
      var spec = this.specResults[key];
      if (spec.result == 'failed') {
      failOutput += JazzHandsReporter.stripHtml(spec.messages) + '\n\n';
      }
    }
    this.output(failOutput);
    throw 'Failed';
  }
